...
本目录是图论专题（存储、遍历、最小生成树、最短路径）。

- `project3.cpp`: 邻接矩阵 + 非递归 DFS 遍历。核心：无向图对称建矩阵，用栈模拟深度优先——每次看栈顶行找一个未访问邻接点就输出、标记、入栈并 `break`，找不到才出栈回溯。
- `project4.cpp`: 邻接表 + BFS 遍历。核心：先用 `vector` 邻接矩阵收边，再从 `n` 到 `1` 倒序头插建邻接表（保证遍历顺序与矩阵一致），队列层层扩展，节点入队即标记已访问。
- `project5.cpp`: 建图并同时输出邻接矩阵和邻接表。核心：无向图两向标记；邻接表输出前对每个顶点的邻接点 `sort` 升序。
- `project6.cpp`: 求连通分量个数 / 判断图是否连通。核心：主函数从 `1` 到 `n` 扫描，遇到未访问节点就 `count++` 并从它 DFS 染色；`count == 1` 输出 `YES`，否则输出 `NO` 和分量数。
- `project7.cpp`: 判断最小生成树是否唯一。核心：Prim 建出一棵 MST 并记录树边，再枚举每条非树边 `(u,v,w)`，在 MST 上 DFS 求 `u→v` 路径的最大边权，若等于 `w` 说明可替换、MST 不唯一。
- `project8.cpp`: Prim 算法求最小生成树权值。核心：`lowcost[]` 记非树点到树的最小权、`closest[]` 记对应端点，每轮选最小后置 `lowcost[k]=0` 并用新点松弛；加入点数不足 `n` 说明图不连通输出 `-1`。
- `project9.cpp`: Kruskal 算法求最小生成树权值。核心：收集所有边按权升序排序，用 `vset[]` 特征位判连通，不同集合才收边并把一侧特征位整体并入另一侧；有效边数 `< n-1` 说明不连通输出 `-1`。注释强调把图对象定义在 `main` 外、引用传参避免爆栈。
- `project10.cpp`: 空文件，当前未实现。
- `project11.cpp`: Floyd 求任意两点最短路（有向图）。核心：三重循环以中间点 `k` 松弛 `dist[i][j]`，重边只保留最小权，最后输出距离矩阵，不可达打印 `INF`。
- `project12.cpp`: Dijkstra 单源最短路（有向图）。核心：`S[]` 标记已确定点，每轮在未确定点里挑 `dist` 最小者加入并松弛邻居，不可达输出 `-1`；注释强调把图对象放到全局静态区避免栈上大数组溢出。
